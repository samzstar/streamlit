...
def create_card_counts_with_names(enriched_df, usage_mode, spark):
    ...
    filepath_card = os.path.join(os.path.dirname(__file__),
        "data/raw/card_ids.csv")
    card_ids_df = load_data(filepath_card, spark)
    ...
    # DataFrame according to what is required, one of "most", "least", "top_ten"
    if usage_mode == "most":
        counts_cards_names_most = counts_cards_names.orderBy(counts_cards_names.card_count.desc())
        counts_cards_names_most = counts_cards_names_most.limit(1)
        counts_cards_names_most = counts_cards_names_most.toPandas()
        
        # Returns name of the most used card as a string
        return counts_cards_names_most.iloc[0, 1]
    
    elif usage_mode == "least":
        
        counts_cards_names_least = counts_cards_names.orderBy(counts_cards_names.card_count.asc())
        counts_cards_names_least = counts_cards_names_least.limit(1)
        counts_cards_names_least = counts_cards_names_least.toPandas()
        
        # Returns name of the least used card as a string
        return counts_cards_names_least.iloc[0, 1]
    
    elif usage_mode == "top_ten":
        ...
        # Returns a pandas dataframe of the top 10 most used cards: card_id, card_name, card_count
        return counts_cards_names_10
    
    else:
        raise "Error: invalid usage mode"
